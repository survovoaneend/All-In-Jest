...
local aij_precision_fix = function(card, x) --hopefully fixes precision
	...
	if snapped < (card.edition.min_mult * 0.01) then
		return (card.edition.min_mult * 0.01)
	end
	if snapped > (card.edition.max_mult * 0.01) then
		return (card.edition.max_mult * 0.01)
	end
	return snapped
end
...
local misprint = {
	...
	loc_vars = function(self, info_queue, card)
		...
		return {
			vars = {
				((card.edition or {}).max_mult or self.config.max_mult) * 0.01,
				((card.edition or {}).min_mult or self.config.min_mult) * 0.01,
			},
		}
	end,
	on_apply = function(card)
		...
		if not card.ability.jest_misprint_active then
			if card.ability.set == "Enhanced" or card.ability.set == "Default" then
				...
				jest_ability_calculate(card, "*", card.edition.mult, {
					h_x_chips = 1,
					Xmult = 1,
					x_chips = 1,
					x_mult = 1,
					...
					bonus_h_x_blind_size = 1, -- latest SMODS stuff
	...
	then
		if tonumber(self.edition.prevmult) ~= self.edition.mult then
			if self.ability.set == "Enhanced" or self.ability.set == "Default" then
				...
				jest_ability_calculate(self, "/", tonumber(self.edition.prevmult), {
					h_x_chips = 1,
					Xmult = 1,
					x_chips = 1,
					x_mult = 1,
				...
				jest_ability_calculate(self, "*", self.edition.mult, {
					h_x_chips = 1,
					Xmult = 1,
					x_chips = 1,
					x_mult = 1,
...
function misprint_atlas(a, card_width, front)
	...
	if not misprinted.atlas then
		...
		SMODS.get_atlas(misprinted.new_name).name = SMODS.get_atlas(misprinted.old_name).name
			.. "_misprinted"
			.. card_width
			.. front
