...
local stellar = {
	...
	loc_vars = function(self, info_queue, card)
		return {
			vars = {
				(card.edition or {}).chips or self.config.chips,
				(card.edition or {}).mult or self.config.mult,
				(card.aij_seal_edition or {}).hands or self.aij_seal_config.hands,
			},
		}
	end,
	calculate = function(self, card, context)
		if context.post_joker or (context.main_scoring and context.cardarea == G.play) then
			local level = context.calculate_seal_edition and math.floor(G.GAME.hands[context.scoring_name].level / 2)
				or G.GAME.hands[context.scoring_name].level
