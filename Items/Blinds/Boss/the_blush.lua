...
function G.FUNCS.can_discard(e)
	...
	if
		(
			(G.GAME.blind.config.blind.key == "bl_aij_the_blush" and not G.GAME.blind.disabled)
			and #G.hand.highlighted ~= 5
		)
		or (
			(G.GAME.blind.config.blind.key == "bl_aij_the_neck" and not G.GAME.blind.disabled)
