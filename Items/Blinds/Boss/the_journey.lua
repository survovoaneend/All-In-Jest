local the_journey = {
		...
		return {
			vars = {
				G.GAME.current_round.aij_the_journey_blind
					and localize(G.GAME.current_round.aij_the_journey_blind.selected_suit, "suits_singular"),
			},
		}
	end,
	...
	calculate = function(self, blind, context)
		...
		if
			...
			and G.GAME.current_round.aij_the_journey_blind
			and not temp
		then
		...
		if
			context.all_in_jest
			and context.all_in_jest.before_after
			and not G.GAME.current_round.aij_the_journey_blind.triggered
			and not temp
		then
			local trigger = false
			...
			if trigger then
				...
				G.E_MANAGER:add_event(Event({
					func = function()
						...
						attention_text({
							scale = 1.0,
							text = localize({ type = "variable", key = "a_aij_win_ante_plus", vars = { 1 } }),
							hold = 4,
							align = "cm",
							offset = { x = 0, y = -2.7 },
							major = G.play,
	...
	defeat = function(self)
		if G.aij_the_journey_stars then
			G.aij_the_journey_stars:fade(1)
		end
		G.E_MANAGER:add_event(Event({
			trigger = "after",
			delay = 1,
