...
function ease_dollars(mod, instant)
	...
	if mod < 0 then
		G.GAME.current_round.aij_aureate_coin_blind.spent_money = G.GAME.current_round.aij_aureate_coin_blind.spent_money
			+ mod
...
local aureate_coin = {
	...
	set_blind = function(self)
		if
			G.GAME.current_round.aij_aureate_coin_blind
			and math.abs(G.GAME.current_round.aij_aureate_coin_blind.spent_money) > 0
		then
			All_in_Jest.ease_blind_requirement(
				math.abs(G.GAME.current_round.aij_aureate_coin_blind.spent_money) * 0.1,
				0
			)
		end
	end,
