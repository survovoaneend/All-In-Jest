...
local polychrome_stake = {
	...
	aij_create_chip = function(self, chip, i)
		...
		if
			unlocked
			and SMODS.RunSelect.Setup.choices[self.key]
			and SMODS.RunSelect.Setup.choices[self.key][chip.key]
		then
		...
		if unlocked then
			chip_card.children.back =
				SMODS.create_sprite(0, 0, 1, 1, chip.atlas, chip.pos, chip_card.config.center.sprite_args)
		else
			chip_card.children.back =
				SMODS.create_sprite(0, 0, 1, 1, "locked_stake", { x = 0, y = 0 }, chip_card.config.center.sprite_args)
		end
			...
			nodes = {
				{
					...
					nodes = {
						{
							n = G.UIT.R,
							config = { align = "bm", minh = 0.5, maxw = G.CARD_W - 0.2 },
							nodes = {
								{
									n = G.UIT.T,
									config = {
										text = unlocked and localize({
											type = "name_text",
											set = "PolyStakeChips",
											key = chip.key,
										}) or "???",
						...
						{
							n = G.UIT.R,
							config = { align = "cm", minh = G.CARD_H - 1.6, maxw = G.CARD_W - 0.2 },
							nodes = {
								{
									n = G.UIT.C,
									config = { align = "cm", minw = G.CARD_W / 2 - 0.1 },
									nodes = {
								...
								{
									n = G.UIT.C,
									config = { align = "cm", minw = G.CARD_W / 2 - 0.1, maxw = G.CARD_W / 2 - 0.1 },
									nodes = {
										{
											n = G.UIT.R,
											...
											nodes = {
												{
													...
													config = {
														id = "points_text",
														object = All_in_Jest.create_mod_points_text(chip.key, stack),
													},
												},
											},
										},
	...
	definition = function(self)
		...
		-- stats
		local deck_name =
			localize({ set = "Back", key = SMODS.RunSelect.Setup.choices.deck_choice, type = "name_text" })
		local pb = G.PROFILES[G.SETTINGS.profile].aij_poly_points or { points = 0, ante = "N/A" }
		local deck_pb = G.PROFILES[G.SETTINGS.profile].deck_usage[SMODS.RunSelect.Setup.choices.deck_choice]
				and G.PROFILES[G.SETTINGS.profile].deck_usage[SMODS.RunSelect.Setup.choices.deck_choice].aij_poly_points
			or { points = 0, ante = "N/A" }
		local stats_def = {
			n = G.UIT.C,
			config = {
				align = "tm",
									...
									nodes = {
										{
											n = G.UIT.T,
											config = {
												text = localize("k_aij_total_points"),
												scale = 0.4,
												colour = G.C.GREY,
											},
										},
									...
									nodes = {
										{
											...
											config = {
												id = "aij_total_points",
												object = All_in_Jest.create_total_points_text(),
											},
										},
										{
											n = G.UIT.O,
											config = {
												id = "aij_total_points_suffix",
												object = DynaText({
													string = { " " .. localize("k_aij_points") },
													colours = { (points > 0) and G.C.UI.TEXT_LIGHT or G.C.GREY },
													shadow = true,
													scale = 0.5,
												}),
											},
									...
									nodes = {
										{
											...
											config = {
												text = deck_name,
												scale = 0.7 / math.max(1.75, string.len(deck_name) / 8),
												colour = G.C.GREY,
											},
										},
									},
													...
													nodes = {
														{
															...
															config = {
																object = DynaText({
																	...
																	colours = {
																		(deck_pb.points > 0) and G.C.FILTER or G.C.GREY,
																	},
																	shadow = true,
																	bump = (deck_pb.points > 0),
																	scale = 0.5,
																}),
															},
														},
													...
													nodes = {
														{
															...
															config = {
																object = DynaText({
																	...
																	colours = {
																		(pb.points > 0) and G.C.FILTER or G.C.GREY,
																	},
																	shadow = true,
																	bump = (pb.points > 0),
																	scale = 0.5,
			...
			nodes = {
				{
					...
					nodes = {
						...
						{
							n = G.UIT.C,
							config = { align = "cm", minw = 4 },
							nodes = {
								{
									n = G.UIT.O,
									config = {
										object = DynaText({
											string = { { ref_table = cycle_args.page_label, ref_value = "text" } },
											scale = 0.5,
											colours = { G.C.WHITE },
											pop_in_rate = 0,
											silent = true,
						...
						{ n = G.UIT.C, config = { align = "cm", minw = 3.75 }, nodes = {} },
						{
							...
							nodes = {
								{
									n = G.UIT.T,
									config = { text = localize("k_aij_disable_all"), scale = 0.4, colour = G.C.WHITE },
								},
							},
						},
					},
		...
		return {
			...
			nodes = {
				...
				{
					n = G.UIT.R,
					config = { align = "tl", padding = 0.05, minh = 0.8 },
					nodes = {
						buttons_def,
					},
				},
...
function All_in_Jest.create_poly_chip_cycle(args)
	...
	for i = 1, args.max_value do
		pips[#pips + 1] = {
			...
			config = {
				...
				r = 0.05,
				id = "pip_" .. i,
				colour = args.current_value >= i and G.C.WHITE or G.C.BLACK,
			},
		}
	end
	...
	local t = {
		n = G.UIT.C,
		config = { align = "cm", padding = 0.1, r = 0.1, colour = G.C.CLEAR, focus_args = { type = "cycle" } },
		nodes = {
			{
				n = G.UIT.C,
				config = {
					...
					minw = 0.4,
					hover = l_active,
					colour = l_active and G.C.RED or G.C.UI.TEXT_DARK,
					button = active and "aij_polychip_cycle" or nil,
					ref_table = args,
					ref_value = "-",
				...
				nodes = {
					{
						...
						config = {
							...
							text = "-",
							scale = 0.4,
							colour = l_active and G.C.UI.TEXT_LIGHT or G.C.UI.TEXT_INACTIVE,
						},
					},
				},
			},
			{
				...
				config = {
					...
					r = 0.1,
					padding = 0.05,
					colour = active and G.C.RED or G.C.UI.TEXT_DARK,
				},
				nodes = {
			...
			},
			{
				...
				config = {
					...
					minw = 0.4,
					hover = r_active,
					colour = r_active and G.C.RED or G.C.UI.TEXT_DARK,
					button = active and "aij_polychip_cycle" or nil,
					ref_table = args,
					ref_value = "+",
					focus_args = { type = "none" },
				...
				nodes = {
					{
						...
						config = {
							...
							text = "+",
							scale = 0.4,
							colour = r_active and G.C.UI.TEXT_LIGHT or G.C.UI.TEXT_INACTIVE,
						},
					},
				},
			},
...
G.FUNCS.aij_polychip_cycle = function(e)
	...
	if from_value == e.config.ref_table.current_value then
		return
	end
	SMODS.RunSelect.Setup.choices["aij_polychrome_settings"][e.config.ref_table.chip_key] =
		e.config.ref_table.current_value
	...
	if text_node then
		...
		text_node.config.object =
			All_in_Jest.create_mod_points_text(e.config.ref_table.chip_key, e.config.ref_table.current_value)
	...
	if total_text_node then
		...
		G.OVERLAY_MENU:get_UIE_by_ID("aij_total_points_suffix").config.object.colours =
			{ has_points and G.C.UI.TEXT_LIGHT or G.C.GREY }
	end
...
function All_in_Jest.create_mod_points_text(key, value)
	...
	if value == 0 then
		...
		return DynaText({
			string = { "+" .. All_in_Jest.PolyStakeChips[key].points[value] },
			colours = { G.C.FILTER },
			shadow = true,
			bump = true,
			scale = 0.5,
...
function All_in_Jest.get_poly_points_color(points)
	local pb = G.PROFILES[G.SETTINGS.profile].aij_poly_points and G.PROFILES[G.SETTINGS.profile].aij_poly_points.points
		or 0
	if points == 0 then
		return G.C.GREY
...
function All_in_Jest.populate_poly_tower(silent)
	if SMODS.config.run_select_performance then
		silent = true
	end
	...
	for _, v in ipairs(All_in_Jest.PolyStakeChip.obj_buffer) do
		if
			SMODS.RunSelect.Setup.choices["aij_polychrome_settings"]
			and SMODS.RunSelect.Setup.choices["aij_polychrome_settings"][v] > 0
		then
			for __ = 1, SMODS.RunSelect.Setup.choices["aij_polychrome_settings"][v] do
				local chip_card =
					Card(tower_area.T.x, tower_area.T.y, 0.99, 0.99, nil, G.P_CENTERS.j_joker, { aij_poly_chip = v })
				chip_card.no_shadow = true
				chip_card.facing = "back"
				chip_card.sprite_facing = "back"
				...
				chip_card.states.drag.can = false
				chip_card.children.back:remove()
				chip_card.children.back = SMODS.create_sprite(
					chip_card.T.x,
					chip_card.T.y,
					chip_card.T.w,
					chip_card.T.h,
					chip_card.config.center.atlas,
					chip_card.config.center.pos,
					chip_card.config.center.sprite_args
				)
				...
				chip_card.params.run_select_stake_tower =
					{ G.P_STAKES["stake_aij_polychrome"].order, "stake_aij_polychrome" }
					...
					G.E_MANAGER:add_event(
						Event({
							...
							func = function()
								...
								if SMODS.RunSelect.Internals.stake_tower.cards then
									SMODS.RunSelect.Internals.stake_tower:draw_card_from(
										SMODS.RunSelect.Internals.stake_tower_holding
									)
								end
								return true
							end,
	...
	local poly_card = SMODS.RunSelect.Pages.stake_choice:create_selection_card(
...
function Card:hover()
	if self.params.aij_poly_chip and (not self.states.drag.is or G.CONTROLLER.HID.touch) and not self.no_ui then
		...
		local function create_info_tooltip(tooltip_data)
			local desc = generate_card_ui(
				tooltip_data,
				{ main = {}, info = {}, type = {}, name = "done", badges = {}, from_detailed_tooltip = true },
				nil,
				tooltip_data.set,
				nil
			)
		...
		for _, v in ipairs(info_queue) do
			table.insert(
				tooltips,
				{ n = G.UIT.R, config = { align = "cm", padding = 0.05 }, nodes = { create_info_tooltip(v) } }
			)
		end
