local twofold = {
	...
	apply = function(self, back)
		...
		for k, v in pairs(G.P_CARDS) do
			if
				not SMODS.add_to_pool(SMODS.Ranks[v.value], { initial_deck = true, suit = v.suit })
				or not SMODS.add_to_pool(SMODS.Suits[v.suit], { initial_deck = true, rank = v.value })
			then
				goto continue
			end
			local _ = nil
			...
			if keep then
				G.GAME.starting_params.extra_cards[#G.GAME.starting_params.extra_cards + 1] =
					{ s = _s, r = _r, e = _e, d = _d, g = _g }
			end
			::continue::
		end
	end,
