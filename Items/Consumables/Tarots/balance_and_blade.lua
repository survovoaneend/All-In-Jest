	...
	use = function(self, card)
		...
		if next(keys) then
			...
			for key, val in pairs(All_in_Jest.get_suits(nil, true)) do
				for k, v in pairs(G.hand.highlighted) do
					if
						v.base.suit ~= key
						and not All_in_Jest.has_patches(v, key)
						and All_in_Jest.has_suit_in_deck(key, true)
					then
						keys[#keys + 1] = key
					end
				end
			...
			if next(keys) then
				...
				if next(keys) then
					...
					cur_suit = pseudorandom_element(
						SMODS.Suits,
						pseudoseed(
							"balance_and_blade_resample_how_in_the_everliving_localthunk_did_you_get_to_this_point"
						)
					)
		...
		G.E_MANAGER:add_event(Event({
			...
			func = function()
				for k, v in pairs(G.hand.highlighted) do
					if
						(v.base.suit ~= cur_suit or SMODS.has_no_suit(v)) and not All_in_Jest.has_patches(v, cur_suit)
					then
						All_in_Jest.add_patch(v, cur_suit, true)
					end
				end
				return true
			end,
