local fabled = {
			...
			G.E_MANAGER:add_event(Event({
				...
				func = function()
					G.GAME.jest_legendary_pool.rate = G.GAME.jest_legendary_pool.rate
						- sleeve.config.increase_legendary_pool_rate
					return true
				end,
			}))
		else
	...
	calculate = function(self, sleeve, context)
		local deck_or_sleeve = (sleeve.config.extra and sleeve.config.extra.should_increase and sleeve)
		...
		if not sleeve.config.increase_legendary_pool_rate then
			...
			if context.buying_card then
				if context.card.ability.set == "Joker" and context.card.config.center.rarity == 4 then
					...
					G.GAME.jest_legendary_pool.rate = G.GAME.jest_legendary_pool.rate
						+ deck_or_sleeve.config.extra.remove_amt
