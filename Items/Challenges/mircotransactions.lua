local mircotransactions = {
	...
	unlocked = function(self) -- Unlocks if player has discovered at least one pit blind
		if
			G
			and G.P_CENTERS
			and G.P_CENTERS["j_aij_gopnik"].discovered == true
			and G.P_CENTERS["j_aij_farceur"].discovered == true
		then
			return true
		end
	end,
...
if not All_in_Jest.config.moons_enabled then
	mircotransactions.restrictions.banned_cards[#mircotransactions.restrictions.banned_cards + 1] =
		{ id = "c_high_priestess" }
	mircotransactions.restrictions.banned_cards[#mircotransactions.restrictions.banned_cards + 1] =
		{ id = "c_aij_gravastar" }
	mircotransactions.restrictions.banned_cards[#mircotransactions.restrictions.banned_cards + 1] =
