...
G.FUNCS.tag_aij_nonstandard_set_enhancement = function(args)
  ...
  local other_enhancement_key = nil
  if G.customize_card.cards[1].aij_nonstandard_other_enhancement_setting ~= nil then
    other_enhancement_key = G.customize_card.cards[1].aij_nonstandard_other_enhancement_setting
  end
  G.E_MANAGER:add_event(Event({
    func = function()
      G.customize_card.cards[1]:set_ability(G.P_CENTERS["c_base"])
      G.customize_card.cards[1]:set_ability(G.P_CENTERS[enhancement_key])
      if other_enhancement_key ~= nil and other_enhancement_key ~= enhancement_key then
        All_in_Jest.set_other_enhancement(G.customize_card.cards[1], other_enhancement_key)
      end
      return true
    end,
  }))
...
G.FUNCS.tag_aij_nonstandard_set_other_enhancement = function(args)
  if G.customize_card.cards[1].config.center.key == "c_base" then return end
  ...
  local enhancement_key = enhancement_pool[args.to_key]
  if enhancement_key == "c_base" then
    G.customize_card.cards[1].aij_nonstandard_other_enhancement_setting = nil
  else
    G.customize_card.cards[1].aij_nonstandard_other_enhancement_setting = enhancement_key
  end
  G.E_MANAGER:add_event(Event({
    func = function()
      if enhancement_key == "c_base" or enhancement_key == G.customize_card.cards[1].config.center.key then
        G.customize_card.cards[1].config.aij_other_center = nil
        G.customize_card.cards[1]:set_sprites(G.P_CENTERS[G.customize_card.cards[1].config.center.key])
      else
        All_in_Jest.set_other_enhancement(G.customize_card.cards[1], enhancement_key)
      end
      return true
    end,
  }))
  G.E_MANAGER:add_event(Event({
    func = function()
      G.customize_card.cards[1].front_hidden = G.customize_card.cards[1]:should_hide_front()
      if not G.customize_card.cards[1].front_hidden then
        G.customize_card.cards[1]:set_sprites(nil, G.customize_card.cards[1].config.card)
      end
      return true
    end,
  }))
...
G.FUNCS.tag_aij_nonstandard_set_paperback_clip = function(args)
  ...
  for sticker_key, _ in pairs(SMODS.Stickers) do
    if
        PB_UTIL.is_paperclip(sticker_key)
        and not PB_UTIL.is_special_clip(string.sub(sticker_key, #"paperback_" + 1, #sticker_key))
    then
...
function create_UIBox_nonstandard(create_func)
  ...
  local suits_option_cycle = create_option_cycle({
    ...
    w = 3,
    options = suit_pool,
    opt_callback = "tag_aij_nonstandard_set_suit",
    current_option = 1,
  })
  ...
  local ranks_option_cycle = create_option_cycle({
    ...
    w = 3,
    options = rank_pool,
    opt_callback = "tag_aij_nonstandard_set_rank",
    current_option = 1,
  })
  ...
  local enhancements_option_cycle = create_option_cycle({
    ...
    scale = 0.8,
    w = 3,
    options = enhancement_pool,
    opt_callback = "tag_aij_nonstandard_set_enhancement",
    current_option = 1,
  })

  local other_enhancements_option_cycle = nil
  if G.GAME.selected_back and G.GAME.selected_back.name == "b_aij_melded" then
    other_enhancements_option_cycle = create_option_cycle({
      label = "Enhancement",
      text_scale = 0.4,
      scale = 0.8,
      w = 3,
      options = enhancement_pool,
      opt_callback = "tag_aij_nonstandard_set_other_enhancement",
      current_option = 1,
    })
  end
  ...
  local editions_option_cycle = create_option_cycle({
    label = "Edition",
    text_scale = 0.4,
    scale = 0.8,
    w = 3,
    options = edition_pool,
    opt_callback = "tag_aij_nonstandard_set_edition",
    current_option = 1,
  })
  ...
  local seals_option_cycle = create_option_cycle({
    label = "Seal",
    text_scale = 0.4,
    scale = 0.8,
    w = 3,
    options = seal_pool,
    opt_callback = "tag_aij_nonstandard_set_seal",
    current_option = 1,
  })
  ...
  if next(SMODS.find_mod("paperback")) then
    ...
    paperback_clip_option_cycle = create_option_cycle({
      label = "Paperclips",
      text_scale = 0.4,
      scale = 0.8,
      w = 3,
      options = paperback_clip_pool,
      opt_callback = "tag_aij_nonstandard_set_paperback_clip",
      current_option = 1,
    })
  end
        ...
        nodes = {
          ...
          },
          {
            ...
            nodes = {
              ...
              other_enhancements_option_cycle,
