local get_valid_jokers = function(exclude)
	...
	if G.jokers then
		for i = 1, #G.jokers.cards do
			if
				G.jokers.cards[i].config.center.key ~= "j_aij_error_404"
				and (G.jokers.cards[i].ability.aij_blueprint_compat or G.jokers.cards[i].config.center.blueprint_compat)
				and G.jokers.cards[i].unique_val ~= exclude
			then
...
local error_404 = {
	...
	update = function(self, card, dt)
		...
		if
			target_joker
			and target_joker ~= card
			and (target_joker.ability.aij_blueprint_compat or target_joker.config.center.blueprint_compat)
		then
