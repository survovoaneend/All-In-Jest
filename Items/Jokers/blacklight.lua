	...
	all_in_jest = {
		animate_func = function(dt, anim, obj, loc, key)
			if G.GAME and G.GAME.round_resets and G.GAME.round_resets.ante then
				...
				if (G.GAME.round_resets.ante % 2) == 0 and math.random(1, 200) == 15 then
				...
				if G.GAME.blacklight_should_flicker and G.GAME.all_in_jest then
					...
					return (loc + anim.flicker_frame) % (anim.frames_per_row or anim.frames),
						math.floor((loc + anim.flicker_frame) / (anim.frames_per_row or anim.frames))
				elseif G.GAME.all_in_jest then
					G.GAME.all_in_jest.AIJAnimated[key].frames = 19
				end
				if (G.GAME.round_resets.ante % 2) == 1 then
					return (loc + 1) % (anim.frames_per_row or anim.frames),
						math.floor((loc + 1) / (anim.frames_per_row or anim.frames))
				end
			end
