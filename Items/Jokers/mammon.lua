	...
	calculate = function(self, card, context)
		if context.individual and context.cardarea == G.play then
			if SMODS.has_enhancement(context.other_card, "m_gold") then
				card.ability.extra.payout_increase_total = card.ability.extra.payout_increase_total
					+ card.ability.extra.payout_increase_mod
				G.GAME.all_in_jest.gold_card_amt = G.GAME.all_in_jest.gold_card_amt
					+ card.ability.extra.payout_increase_mod
...
SMODS.Enhancement:take_ownership("gold", {
	update = function(self, card, dt)
		...
		local factor = (card.ability and card.ability.jest_charged_applied and card.ability.jest_charged_applied.factor)
			or 1
