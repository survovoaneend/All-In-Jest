local borra = {
	...
	loc_vars = function(self, info_queue, card)
		...
		return {
			main_start = {
				...
				{
					n = G.UIT.T,
					config = { ref_table = card.ability.extra, ref_value = "mult_mod", colour = G.C.MULT, scale = 0.32 },
				},
	...
	calculate = function(self, card, context)
		local triggers = (context.setting_ability and not context.unchanged)
			or (context.individual and context.cardarea == G.play)
			or context.press_play
			or context.ante_change
			or context.joker_type_destroyed
			or context.card_added
			or context.playing_card_added
			or context.tag_triggered
			or context.tag_added
			or context.skip_blind
			or context.poker_hand_changed
			...
			or (context.destroy_card and context.cardarea == G.play)
	...
	generate_ui = function(self, info_queue, card, desc_nodes, specific_vars, full_UI_table)
		...
		if target.vars.aij_multi_copier_card then
			...
			new_ability[card.config.center.key .. "_compat_ui"] = new_ability[card.config.center.key .. "_compat_ui"]
				or ""
			...
			local main_end = (card.area and card.area == G.jokers)
					and {
						{
							n = G.UIT.C,
									...
									nodes = {
										{
											n = G.UIT.T,
											config = {
												text = " " .. (compatible and localize("k_compatible") or localize(
													"k_incompatible"
												)) .. " ",
		...
		if desc_nodes == full_UI_table.main and not full_UI_table.name then
			full_UI_table.name = self.set == "Enhanced" and "temp_value"
				or localize({
					type = "name",
					set = res.name_set or target.set,
		...
		elseif desc_nodes ~= full_UI_table.main and not desc_nodes.name then
			...
			if
				(not full_UI_table.from_detailed_tooltip or full_UI_table.info[1] == desc_nodes)
			...
			then
				...
				localize({
					...
					no_spacing = true,
					no_bump = true,
					vars = res.name_vars or target.vars,
				})
