local you_broke_it = {
	...
	loc_vars = function(self, info_queue, card)
		...
		return {
			vars = {
				localize(
					G.GAME.current_round.jest_you_broke_it_card and G.GAME.current_round.jest_you_broke_it_card.rank
						or "Ace",
					"ranks"
				),
				localize({
					type = "name_text",
					set = "Enhanced",
					key = G.GAME.current_round.jest_you_broke_it_card
							and G.GAME.current_round.jest_you_broke_it_card.enhancement
						or "m_bonus",
				}),
			},
		}
		...
		if context.before and not context.blueprint then
			...
			if context.scoring_hand and #context.scoring_hand > 0 then
				for k, v in ipairs(context.scoring_hand) do
					if
						v:get_id() == G.GAME.current_round.jest_you_broke_it_card.id
						and not v.debuff
