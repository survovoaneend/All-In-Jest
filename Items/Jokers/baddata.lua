local baddata = {
	...
	all_in_jest = {
		...
		layer_funcs = {
			...
			soul_pos = function(anim, obj, loc)
				...
				anim.cur_frames = anim.cur_frames or {}
				anim.cur_frames[#anim.cur_frames + 1] = loc
				anim.special_frames = anim.special_frames
					or {
						{ 2, 19 },
						{ 3, 19 },
						{ 4, 19 },
				...
				if anim.other_cur_frame >= anim.other_temp_hold then
					if not anim.temp_hold_2 then
						...
						anim.held_frame = anim.soul_pos_pos[2] * (anim.frames_per_row or anim.frames)
							+ anim.soul_pos_pos[1]
					end
	...
	loc_vars = function(self, info_queue, card)
		...
		elseif card.ability.extra.effect == "dollars" then
			...
			loc_mult = string.upper(
				string.sub(localize("k_aij_overdesigned_earn_prefix", "extra_joker_dictionary"), 1, 1)
			) .. string.sub(localize("k_aij_overdesigned_earn_prefix", "extra_joker_dictionary"), 2)
		end
		return {
