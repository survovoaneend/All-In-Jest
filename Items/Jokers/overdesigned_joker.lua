...
local overdesigned_joker = {
	...
	loc_vars = function(self, info_queue, card)
		local suits = {
			Spades = "overdesigned_spade",
			Hearts = "overdesigned_heart",
			Clubs = "overdesigned_club",
			Diamonds = "overdesigned_diamond",
		}
		for suit_key, loc_key in pairs(suits) do
			info_queue[#info_queue + 1] = {
				...
				vars = {
					round_hundredth(
						jest_get_overdesigned_ability[suit_key].amount * (card.ability.silver_multiplier_buff / 100)
					),
				},
			}
		end
		...
		return {
			vars = {
				...
				current_suit_ability.extra.prefix
						and localize(current_suit_ability.extra.prefix, "extra_joker_dictionary")
					or "",
				current_suit_ability.extra.symbol
						and localize(current_suit_ability.extra.symbol, "extra_joker_dictionary")
					or "",
				current_suit_ability.extra.suffix
						and localize(current_suit_ability.extra.suffix, "extra_joker_dictionary")
					or "",
				colours = {
					...
					(card.ability.suit == "Diamonds" and current_suit_ability.extra.colours.suit)
