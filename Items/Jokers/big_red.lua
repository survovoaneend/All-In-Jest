...
local has_attribute = function(center, attribute)
	if not SMODS.Attributes[attribute] or not center.attributes then
		return false
	end
	if center.attributes[attribute] then
		return true
	end
	for _, att in ipairs(SMODS.Attributes[attribute].alias or {}) do
		if center.attributes[att] then
			return true
		end
	end
	return false
...
function SMODS.add_to_pool(prototype_obj, ...)
	if prototype_obj.set == "Joker" then
		if
			#SMODS.find_card("j_aij_big_red") > 0
			and #SMODS.find_card("j_aij_little_boy_blue") > 0
			and not (is_mult_joker(prototype_obj) or is_chips_joker(prototype_obj))
		then
			return false
		elseif
			#SMODS.find_card("j_aij_big_red") > 0
			and not (is_mult_joker(prototype_obj) and not is_chips_joker(prototype_obj))
		then
			return false
		elseif
			#SMODS.find_card("j_aij_little_boy_blue") > 0
			and not (is_chips_joker(prototype_obj) and not is_mult_joker(prototype_obj))
		then
			return false
		end
	end
