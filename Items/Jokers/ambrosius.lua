local ambrosius = {
	...
	add_to_deck = function(self, card, from_debuff)
		G.GAME.modifiers.arcana_booster_size_mod = (G.GAME.modifiers.arcana_booster_size_mod or 0)
			+ tonumber(card.ability.extra.prev_pack_size)
	end,
		...
		if G.GAME and G.GAME.round_resets.ante and G.hand then
			...
			if has_card then
				...
				if pack_size ~= prev_pack_size then
					G.GAME.modifiers.arcana_booster_size_mod = (G.GAME.modifiers.arcana_booster_size_mod or 0)
						+ (pack_size - tonumber(card.ability.extra.prev_pack_size))
