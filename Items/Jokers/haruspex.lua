...
local haruspex = {
	...
	loc_vars = function(self, info_queue, card)
		local to_draw = G.deck and #G.deck.cards > 0 and math.min(card.ability.extra.cards_to_draw, #G.deck.cards)
			or card.ability.extra.cards_to_draw
		return {
			vars = {
	...
	all_in_jest = {
		can_use_ability = function(self, card, context)
			if
				...
				and G.deck.cards
				and #G.deck.cards > 0
				and G.GAME.current_round.discards_left >= card.ability.extra.discards_to_spend
				...
				G.E_MANAGER:add_event(Event({
					...
					func = function()
						...
						G.FUNCS.overlay_menu({
							config = { no_esc = true },
							definition = SMODS.jest_no_back_card_collection_UIBox(G.deck.cards, { 5, 5, 5 }, {
								no_materialize = true,
								from_area = true,
