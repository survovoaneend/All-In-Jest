      ...
      if G.play ~= nil and has_red_sky then
        for _, card in ipairs(G.play.cards) do
          if context.before then
            ...
            if saved == nil then
              local _, cval                   = jest_ability_get_items(card, "nil", 2, { extra_value = true },
                { "nominal" }, false, true, "base")
              local _, hval                   = jest_ability_get_items(card, "nil", 2, { extra_value = true },
                { "h_chips" }, false, true, "ability.extra")
              local _, bval                   = jest_ability_get_items(card, "nil", 2, { extra_value = true },
              { "bonus" },
