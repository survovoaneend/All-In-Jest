				...
				func = function()
					...
					for k, v in pairs(G.shop_jokers.cards) do
						if v.config.center.set == "Joker" then
							...
							if
								(G.GAME.jest_legendary_pool.in_shop and force_rarity <= 3 and force_rarity >= 1)
								or (force_rarity <= 2 and force_rarity >= 1)
							then
							...
							if pseudorandom("four_leaf_clover" .. index) < (fours * mult / 100) then
								if force_rarity ~= (v.config.center.rarity or 1) then
									All_in_Jest.reroll_joker(
										v,
										nil,
										"four_leaf_clover" .. index,
										nil,
										{ forced_rarity = force_rarity, shop_ui = G.shop_jokers }
									)
