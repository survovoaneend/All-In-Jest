local select_random_valid_joker = function()
	...
	for _, key in pairs(get_current_pool("Joker")) do
		...
		if
			...
			and not (center.all_in_jest and center.all_in_jest.use_ability)
			and not (center.config.j_aij_joker_png_compat == false)
			and not center.config[key] -- Disallow copiers, bandaid fix for a bug
		then
			jokers[#jokers + 1] = key
		end
	end
...
local joker_png = {
	...
	all_in_jest = {
		...
		use_ability = function(self, card, args)
			...
			if not args.free then
				card.ability.j_aij_joker_png.cost = card.ability.j_aij_joker_png.cost
