	...
	loc_vars = function(self, info_queue, card)
		...
		if #card.ability.extra.banned_cards <= 0 then
			nodes[#nodes + 1] = {
				n = G.UIT.T,
				config = {
					text = localize("k_aij_blacklist_empty", "extra_joker_dictionary"),
					colour = G.C.RED,
					scale = 0.32,
				},
			}
						...
						nodes = {
							{
								n = G.UIT.T,
								config = {
									text = localize("k_aij_blacklist_and", "extra_joker_dictionary") .. " ",
									colour = G.C.L_BLACK,
									scale = 0.32,
								},
							},
							{ n = G.UIT.T, config = { text = more, colour = G.C.FILTER, scale = 0.32 } },
							{
								n = G.UIT.T,
								config = {
									text = " " .. localize("k_aij_blacklist_more", "extra_joker_dictionary") .. "...",
									colour = G.C.L_BLACK,
									scale = 0.32,
								},
							},
