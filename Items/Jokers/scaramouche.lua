local scaramouche = {
	...
	loc_vars = function(self, info_queue, card)
		...
		for k, v in pairs(G.GAME.hands) do
			if SMODS.is_poker_hand_visible(k) and k ~= card.ability.scaramouche_poker_hand then
	...
	calculate = function(self, card, context)
		if context.end_of_round and context.main_eval and not context.blueprint then
			local _poker_hands = {}
			for k, v in pairs(G.GAME.hands) do
				if SMODS.is_poker_hand_visible(k) and k ~= card.ability.extra.scaramouche_poker_hand then
					_poker_hands[#_poker_hands + 1] = k
				end
			end
			card.ability.extra.scaramouche_poker_hand = pseudorandom_element(_poker_hands, pseudoseed("scaramouche"))
