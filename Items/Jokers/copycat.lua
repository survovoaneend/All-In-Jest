	...
	calculate = function(self, card, context)
		if context.end_of_round and not context.individual and not context.repetition then
			...
			if card.ability.rounds_held == card.ability.total_rounds then
				local eval = function(eval_card)
					return not eval_card.REMOVED
				end
				juice_card_until(card, eval, true)
			end
			return {
				message = (card.ability.rounds_held < card.ability.total_rounds)
						and (card.ability.rounds_held .. "/" .. card.ability.total_rounds)
		...
		if context.selling_self and (card.ability.rounds_held >= card.ability.total_rounds) then
			local eval = function(eval_card)
				return (eval_card.ability.loyalty_remaining == 0) and not G.RESET_JIGGLES
			end
			juice_card_until(card, eval, true)
				...
				repeat
					...
					for i, joker_key in ipairs(jokers_pool) do
						if
							...
							and G.P_CENTERS[joker_key].attributes
							and G.P_CENTERS[joker_key].attributes["copying"]
						then
