local angel_number = {
	...
	calculate = function(self, card, context)
			...
			if tonumber(sevens) > 0 then
				...
				return {
					message = localize({
						type = "variable",
						key = "a_aij_odds",
						vars = { sevens * card.ability.extra.numeratormod },
					}),
					colour = G.C.GREEN,
				}
			end
		...
		if (context.all_in_jest and context.all_in_jest.drew_cards) or (context.end_of_round and context.main_eval) then
