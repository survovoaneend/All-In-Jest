	...
	calculate = function(self, card, context)
		...
		if context.individual and context.cardarea == G.play and not context.blueprint then
			if SMODS.has_enhancement(context.other_card, "m_stone") then
				local ability_table = (context.other_card.config.center.key == "m_stone" and context.other_card.ability)
					or (context.other_card.config.aij_other_center and context.other_card.config.aij_other_center.center.key == "m_stone" and context.other_card.config.aij_other_center.ability)
					or nil
				...
				if ability_table and (ability_table.bonus - card.ability.extra.chips_mod) >= 0 then
					...
					base_ability_table.chipped_joker_chips_taken = base_ability_table.chipped_joker_chips_taken
						+ chip_increase
		...
		if context.destroy_card then
			for k, v in pairs(context.scoring_hand) do
				if SMODS.has_enhancement(v, "m_stone") then
					local ability_table = (v.config.center.key == "m_stone" and v.ability)
						or (v.config.aij_other_center and v.config.aij_other_center.center.key == "m_stone" and v.config.aij_other_center.ability)
						or nil
