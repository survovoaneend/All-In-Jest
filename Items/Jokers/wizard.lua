		...
		if context.before and not context.blueprint then
			...
			for _, v in ipairs(context.scoring_hand) do
				if
					...
					and SMODS.pseudorandom_probability(card, "wizard", 1, card.ability.extra.odds)
				then
			...
			if #upgraded > 0 then
				for _, v in ipairs(upgraded) do
					...
					if type_roll > 0.66 then
						new_ability = G.P_CENTERS.m_bonus
					elseif type_roll > 0.33 then
						new_ability = G.P_CENTERS.m_lucky
					end
