local memory_card = {
    ...
    remove_from_deck = function(self, card, from_debuff)
        G.GAME.jest_stored_memory_card = nil
    end,
      ...
      if context.before and context.cardarea == G.jokers then
        if G.GAME.current_round.hands_played == 0 then
            if #context.full_hand == 1 then
                G.GAME.jest_stored_memory_card = context.full_hand[1]
                return {
    ...
    -- I can't line up the card correctly T_T
    --generate_ui = function(self, info_queue, cardd, desc_nodes, specific_vars, full_UI_table)
    --    if G.GAME.jest_stored_memory_card ~= nil and cardd.area == G.jokers then
    --        local cards = {}
    --        local card = copy_card(G.GAME.jest_stored_memory_card, nil, nil, G.playing_card)
