local office_assistant = {
	...
	all_in_jest = {
		ability_cost = function(self, card)
			...
			if right_card then
				if
					right_card.ability.rental
					and right_card.ability.perishable
					and G.GAME.dollars >= right_card.config.center.cost
				then
					return right_card.config.center.cost
				elseif
					right_card.ability.rental
					and right_card.ability.perishable
					and (right_card.ability.perish_tally < G.GAME.perishable_rounds or 5)
				then
					return 3
				elseif right_card.ability.rental then
					return right_card.config.center.cost
				elseif
					right_card.ability.perishable and (right_card.ability.perish_tally < G.GAME.perishable_rounds or 5)
				then
					return 3
				end
			end
		...
		can_use_ability = function(self, card, context)
			...
			for k, v in pairs(G.jokers.cards) do
				if v == card and G.jokers.cards[k + 1] and G.jokers.cards[k + 1].ability then
					if
						(
							G.jokers.cards[k + 1].ability.perishable
							and G.GAME.dollars >= 3
							and G.jokers.cards[k + 1].ability.perish_tally < (G.GAME.perishable_rounds or 5)
						)
						or (
							G.jokers.cards[k + 1].ability.rental
							and G.GAME.dollars >= G.jokers.cards[k + 1].config.center.cost
						)
					then
		...
		use_ability = function(self, card, args)
			...
			elseif right_card.ability.perishable and ((G.GAME.dollars >= 3) or args.free) then
				...
				card_eval_status_text(
					...
					nil,
					nil,
					{ message = localize("aij_refreshed") .. "!" }
				)
			end
		end,
	},
