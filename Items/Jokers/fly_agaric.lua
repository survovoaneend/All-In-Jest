local fly_agaric = {
	...
	calculate = function(self, card, context)
		if
			context.all_in_jest
			and context.all_in_jest.joker_ability_used
			and not context.all_in_jest.args.retriggered
		then
		...
		if G.jokers then
			if #G.jokers.cards > 0 then
				for i = 1, #G.jokers.cards do
					if
						G.jokers.cards[i].config.center.all_in_jest
						and G.jokers.cards[i].config.center.all_in_jest.use_ability
					then
						return true
					end
				end
