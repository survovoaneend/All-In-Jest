local clay_joker = {
	...
	set_ability = function(self, card, initial, delay_sprites)
		for index = 1, #G.GAME.all_in_jest.previously_destroyed_jokers do
			...
			All_in_Jest.add_copied_joker(
				card,
				copied_center,
				G.GAME.all_in_jest.previously_destroyed_jokers[index].ability,
				true
			)
		end
	end,
	...
	if G.jokers and card.ability.set == "Joker" then
		...
		for _, clay_joker_card in ipairs(clay_joker_cards) do
			if clay_joker_card ~= card then
				local destroyed_joker =
					G.GAME.all_in_jest.previously_destroyed_jokers[#G.GAME.all_in_jest.previously_destroyed_jokers]
				...
				G.E_MANAGER:add_event(Event({
					func = function()
						All_in_Jest.add_copied_joker(
							clay_joker_card,
							copied_center,
							destroyed_joker.ability,
							not clay_joker_card.added_to_deck
						)
						return true
					end,
				}))
