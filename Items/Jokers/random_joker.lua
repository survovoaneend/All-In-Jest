function aij_outline_image(imagedata, outline_color)
	...
	local r, g, b, a = outline_color[1], outline_color[2], outline_color[3], outline_color[4] or 1
...
local function aij_upscale_imagedata(src, scale)
	...
	for y = 0, h - 1 do
		for x = 0, w - 1 do
			...
			for oy = 0, scale - 1 do
				for ox = 0, scale - 1 do
					dst:setPixel(x * scale + ox, y * scale + oy, r, g, b, a)
				end
			end
		end
	end
			...
			if outer_prefix ~= "" then
				...
				if prefixes[o_index].no_amt then
					for j = 0, (prefixes[o_index].no_amt - 1) do
						index = #random_joker_files[location] + 1
						random_joker_files[location][index] =
							{ data = nooutline_data, outline = false, pos = { x = j, y = i } }
					end
				end
			end
		end
