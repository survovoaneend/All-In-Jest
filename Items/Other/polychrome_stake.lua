	...
	if G.GAME.aij_poly_points then
		for k, v in pairs(G.jokers.cards) do
			if v.config.center_key and v.ability.set == "Joker" then
				...
				G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].aij_poly_points = math.max(
					G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].aij_poly_points or 0,
					G.GAME.aij_poly_points
				)
			end
		end
		G:save_settings()
...
function set_deck_win()
	if G.GAME.aij_poly_points then
		G.PROFILES[G.SETTINGS.profile].aij_poly_points = G.PROFILES[G.SETTINGS.profile].aij_poly_points
			or { points = 0, ante = G.GAME.round_resets.ante - 1 }
		if G.GAME.aij_poly_points > G.PROFILES[G.SETTINGS.profile].aij_poly_points.points then
			...
			if not G.PROFILES[G.SETTINGS.profile].all_unlocked then
				for _, v in ipairs(All_in_Jest.PolyStakeChip.obj_buffer) do
					...
					if
						not chip.unlocked
						and chip.point_requirement
						and (prev_points < chip.point_requirement)
						and (G.PROFILES[G.SETTINGS.profile].aij_poly_points.points >= chip.point_requirement)
					then
		...
		then
			...
			G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key] = G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key]
				or convert_usage_entry({
					count = 1,
			...
			G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].aij_poly_points = G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].aij_poly_points
				or { points = 0, ante = G.GAME.round_resets.ante }
...
function check_and_set_high_score(score, amt)
	...
	if
		G.GAME.aij_poly_points
		and G.GAME.aij_poly_points > 0
		and score == "furthest_ante"
		and type(amt) == "number"
		and amt > 8
	then
		G.PROFILES[G.SETTINGS.profile].aij_poly_points = G.PROFILES[G.SETTINGS.profile].aij_poly_points
			or { points = 0, ante = amt - 1 }
		if G.GAME.aij_poly_points >= G.PROFILES[G.SETTINGS.profile].aij_poly_points.points then
			G.PROFILES[G.SETTINGS.profile].aij_poly_points.ante =
				math.max(G.PROFILES[G.SETTINGS.profile].aij_poly_points.ante, amt - 1)
		end
		if
			G.GAME.selected_back
			and G.GAME.selected_back.effect
		...
		then
			...
			G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].aij_poly_points = G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].aij_poly_points
				or { points = 0, ante = amt - 1 }
			if G.GAME.aij_poly_points >= G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].aij_poly_points.points then
				G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].aij_poly_points.ante =
					math.max(G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].aij_poly_points.ante, amt - 1)
			end
		end
	end
end
	...
	if G.GAME.aij_poly_points and G.GAME.aij_poly_points > 0 then
		...
		local applied_stake = {
			...
			nodes = {
				{
					n = G.UIT.C,
					config = { align = "cm", padding = 0.1, r = true, colour = G.C.BLACK },
					nodes = {
					...
					nodes = {
						{
							...
							config = {
								align = "cm",
								padding = 0.05,
								colour = get_stake_col(_stake_center.order),
								r = 0.05,
								stretch = true,
								diff = 2.4,
							},
							nodes = {
								{
									...
									config = {
										align = "cm",
										padding = 0.05,
										colour = adjust_alpha(G.C.WHITE, 0.95),
										r = 0.05,
										minw = 5.5,
										stretch = true,
										diff = 2.5,
									...
									nodes = {
										{
											n = G.UIT.R,
											config = { align = "cm", padding = 0.03, minh = 0.7, minw = 3.8 },
											nodes = { transparent_multiline_text(_stake_desc) },
										},
									},
								},
							},
											...
											nodes = {
												{
													n = G.UIT.R,
													config = { align = "cm", padding = 0.03, minh = 0.7, minw = 3.8 },
													nodes = { transparent_multiline_text(chip_desc) },
												},
											},
										},
									},
		...
		local current_col = {
			{
				n = G.UIT.C,
				config = { align = "tl", r = true, colour = G.C.BLACK },
				nodes = {
					applied_stake,
		...
		return {
			n = G.UIT.ROOT,
			config = { align = "cm", colour = G.C.CLEAR, r = 0.1, padding = 0.1 },
			nodes = {
				{
					n = G.UIT.R,
