		...
		if not effect.remove_default_message then
			if effect.aij_set_mult_message then
				...
				card_eval_status_text(
					...
					percent,
					nil,
					{ message = localize({ type = "variable", key = "a_aij_mult_equal", vars = { amount } }) }
				)
			end
		end
		return true
		...
		if not effect.remove_default_message then
			if effect.aij_set_mult_message then
				...
				card_eval_status_text(
					...
					percent,
					nil,
					{ message = localize({ type = "variable", key = "a_aij_chips_equal", vars = { amount } }) }
				)
			end
		end
		return true
