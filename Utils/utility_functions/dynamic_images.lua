...
function aij_recolour_atlas(old_colour, new_colour, base_atlas, front, args)
	...
	local new_atlas_name = base_atlas.name
		.. "_aij_recoloured_"
		.. (r1 * 256 ^ 3 + g1 * 256 ^ 2 + b1 * 256)
		.. "_"
		.. (r2 * 256 ^ 3 + g2 * 256 ^ 2 + b2 * 256)

	if not SMODS.get_atlas(new_atlas_name) then
		local atlas_image_data = args.image_data
			or (base_atlas and base_atlas.image_data and base_atlas.image_data:clone())
		...
		if args.return_val then
			return atlas_image_data
		end
		...
		if args.return_val then
			return SMODS.get_atlas(new_atlas_name).image_data
		end
		return SMODS.get_atlas(new_atlas_name)
	end
end
...
function aij_recolour_pixel(x, y, r, g, b, a, old_colour, new_colour, tolerance, args)
	...
	if
		args.skip_check
		or (
			math.abs(r - old_colour[1]) <= tolerance
			and math.abs(g - old_colour[2]) <= tolerance
			and math.abs(b - old_colour[3]) <= tolerance
		)
	then
		if args.return_pixel then
			return args.return_pixel(r, g, b, a, old_colour, new_colour, args)
		end
		...
		for y = by0, by1 - 1, scale do
			...
			if a > 0 then
				...
				if v < 0.999 then -- Don't count pure white
					if s < saturation_low then
						saturation_low = s
					end
					if s > saturation_high then
						saturation_high = s
					end
				end
			end
