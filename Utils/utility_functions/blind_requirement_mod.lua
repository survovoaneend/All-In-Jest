...
function All_in_Jest.ease_blind_requirement(mod_mult, mod_add, skip_animation)
	...
	local desired_chip_amount = (original_chips / original_mult) * (current_mult + mod_mult)
		+ mod_add
		+ previously_added
...
function All_in_Jest.aij_refresh_boss_blind()
	...
	if G.SHOP_SIGN and next(SMODS.find_mod("unBlindShopGUI")) then
		...
		G.SHOP_SIGN = UIBox({
			definition = {
				n = G.UIT.ROOT,
				config = { colour = G.C.CLEAR, align = "bm" },
				nodes = {
...
function Blind:set_blind(blind, reset, silent)
	if blind and not reset then
		self.aij_original_chips = get_blind_amount(G.GAME.round_resets.ante)
			* blind.mult
