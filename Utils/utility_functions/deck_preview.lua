				...
				for _, v in pairs(area.cards) do
					if v and type(v) == "table" and not v.debuff then
						...
						if v.ability[v.config.center.key] and v.ability[v.config.center.key].copied_joker_abilities then
							for index = #v.ability[v.config.center.key].copied_joker_abilities, math.max(
								1,
								#v.ability[v.config.center.key].copied_joker_abilities
									- v.ability[v.config.center.key].copy_limit
									+ 1
							), -1 do
								max_future_sense = math.max(
									max_future_sense,
									v.ability[v.config.center.key].copied_joker_abilities[index].future_sense
								)
							end
						end
					end
...
AllInJest.card_area_preview = function(cardArea, desc_nodes, config)
	...
	local uiEX = {
		...
		nodes = {
			{
				n = G.UIT.R,
				config = { padding = padding, r = 0.12, colour = lighten(G.C.JOKER_GREY, 0.5), emboss = 0.07 },
				nodes = {
