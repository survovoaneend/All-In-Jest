...
function SMODS.never_scores(card)
	if card.config.aij_other_center and card.config.aij_other_center["center"] then
		if card.config.aij_other_center["center"].never_scores then
			return true
		end
	end
...
function All_in_Jest.get_enhancements_quantum_hook(card, extra_only, enhancements)
	if
		...
		and card.config.aij_other_center["center"]
		and card.config.aij_other_center["center"].key
	then
...
function Card:get_chip_bonus()
	...
	if
		self.config.aij_other_center
		and self.config.aij_other_center["center"]
		and self.config.aij_other_center["ability"]
	then
