...
function All_in_Jest.counts_as_all_suits(card)
	if G.jokers then
		...
		if glass_joker and SMODS.has_enhancement(card, "m_glass") then
			return true
		end
...
function All_in_Jest.add_tag_to_shop(key, price)
	...
	if tag_key == "tag_orbital" then
		...
		tag.ability.orbital_hand =
			pseudorandom_element(available_hands, pseudoseed(card.ability.booster_pos .. "_orbital"))
	...
	for _, center_key in ipairs(pool) do
		...
		if
			...
			and (
				(extra.type ~= "Joker")
				or (
					victim_rarity == (vanilla_rarities[center_data.rarity] or center_data.rarity or "Common")
					and (center_data.unlocked or G.GAME.modifiers.all_jokers_unlocked or center_data.rarity == 4)
				)
			)
		then
			if center_data.key ~= victim_key then
				if not center_data.demo and not center_data.wip then
					...
					if center_data.in_pool and type(center_data.in_pool) == "function" then
						if not center_data:in_pool() then
							can_add = false
						end
					end
					if can_add then
...
function All_in_Jest_format_destroy(center_text)
	...
	local function add_red_text(text, start_index, end_index, base_format)
		...
		return string.sub(text, 1, start_index - 1)
			.. destroyed_format
			.. string.sub(text, start_index, end_index)
			.. base_format
				...
				for _, text in ipairs(destroy_texts) do
					...
					if start_index then
						...
						if not already_processed then
							...
							local applied_formatting = string.reverse(
								string.match(string.reverse(string.sub(line, 1, start_index - 1)), "}.-{") or "}{"
							)
							...
							if
								not (
									string.match(applied_formatting, "C:red")
									or string.match(applied_formatting, "C:inactive")
								)
							then
								local t = {
		...
		for line_index, _ in ipairs(found_strings[box_index]) do
			for _, t in ipairs(found_strings[box_index][line_index]) do
				...
				if one_box then
					...
					center_text[box_index][line_index] =
						add_red_text(center_text[box_index][line_index], start_index, end_index, base_format)
				end
			end
		end
	end
...
function Card:All_in_Jest_set_seal_edition(edition, immediate, silent, delay)
	...
	if not edition_type or edition_type == "base" then
		if self.aij_seal_edition == nil then
			return
		end
...
function dynatext_aij_draw(self)
	...
	if self.config.marquee and self.config.marquee ~= "no" then
		...
		if
			self.dt
			and (self.dt - self.config.hold) / self.config.scroll_speed
				> (#self.strings[self.focused_string].letters + math.ceil(padding / 4))
		then
			self.dt = 0
		end
		if self.dt and self.dt > self.config.hold then
			start_index = 1
				+ (
					math.floor((self.dt - self.config.hold) / self.config.scroll_speed)
					% (#self.strings[self.focused_string].letters + math.ceil(padding / 4))
				)
		end
	...
	if self.shadow then
		...
		love.graphics.translate(
			self.strings[self.focused_string].W_offset + self.text_offset.x * self.font.FONTSCALE / G.TILESIZE,
			self.strings[self.focused_string].H_offset + self.text_offset.y * self.font.FONTSCALE / G.TILESIZE
		)
		...
		for k = start_index, end_index do
			...
			if
				self.config.text_effect
				and SMODS.DynaTextEffects[self.config.text_effect]
				and type(SMODS.DynaTextEffects[self.config.text_effect].draw_shadow) == "function"
			then
				SMODS.DynaTextEffects[self.config.text_effect].draw_shadow(self, k, letter) -- shadow
			else
				local letter_shaders = SMODS.resolve_ui_shaders(
					self,
					self.states.visible and letter.shader or (self.shaders and self.shaders[k % #self.shaders + 1]),
					nil
				)
				for _, v in ipairs(letter_shaders) do
					if v then
						self:set_letter_shader(v.shader, v.send, true, letter)
					end
					love.graphics.draw(
						letter.letter,
						0.5 * (letter.dims.x - letter.offset.x) * self.font.FONTSCALE / G.TILESIZE
							- self.shadow_parrallax.x * self.scale / G.TILESIZE,
						0.5 * letter.dims.y * self.font.FONTSCALE / G.TILESIZE
							- self.shadow_parrallax.y * self.scale / G.TILESIZE,
						letter.r or 0,
	...
	_shadow_norm.x, _shadow_norm.y =
		self.shadow_parrallax.x / math.sqrt(
			self.shadow_parrallax.y * self.shadow_parrallax.y + self.shadow_parrallax.x * self.shadow_parrallax.x
		) * self.font.FONTSCALE / G.TILESIZE,
		self.shadow_parrallax.y / math.sqrt(
			self.shadow_parrallax.y * self.shadow_parrallax.y + self.shadow_parrallax.x * self.shadow_parrallax.x
		) * self.font.FONTSCALE / G.TILESIZE
	...
	for k = start_index, end_index do
		...
		if
			self.config.text_effect
			and SMODS.DynaTextEffects[self.config.text_effect]
			and type(SMODS.DynaTextEffects[self.config.text_effect].draw_letter) == "function"
		then
			...
			for _, v in ipairs(letter_shaders) do
				if v then
					self:set_letter_shader(v.shader, v.send, false, letter)
				end
				love.graphics.draw(
					letter.letter,
	...
	while j <= #reps do
		...
		if context.cardarea == G.hand and (next(effects) or next(flags)) then
			SMODS.calculate_context({
				aij_held_effect_triggered = true,
				card = card,
				effects = next(effects) and effects or flags,
			})
		end
