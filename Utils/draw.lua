        ...
        if self.config.center and self.config.center.all_in_jest and self.config.center.all_in_jest.soul_layers then
            for k, v in pairs(self.config.center.all_in_jest.soul_layers) do
                ...
                if self.children[k] then
                    self.children[k]:draw_shader("dissolve", 0, nil, nil, self.children.center, scale_mod, rotate_mod, nil, self.config.center.all_in_jest.soul_layers[k].moving and 0.1 + 0.03*math.cos(1.8*G.TIMERS.REAL) or 0.1, nil, 0.6)
