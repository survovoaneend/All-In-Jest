function All_in_Jest.get_inherent_effects(card, type, amt_only, keys)
	if
		card
		and card.aij_inherent_effects
		and card.aij_inherent_effects[type .. "s"]
		and #card.aij_inherent_effects[type .. "s"] > 0
	then
		local effects = {}
		local amt = 0
...
function All_in_Jest.set_other_enhancement(card, enhancement)
	if not G.P_CENTERS[enhancement] then
		return
	end -- enhancement must exist
	if enhancement == card.config.center.key then
		return
	end -- enhancement must not be duplicate of main enhancement
...
function All_in_Jest.get_enhancement_z_order(center)
	...
	if
		center.all_in_jest
		and center.all_in_jest.multi_enhancement_z_order
		and type(center.all_in_jest.multi_enhancement_z_order) == "number"
	then
...
function All_in_Jest.get_multi_enhancement_atlas(center, other_center)
	...
	local has_sprite = aij_check_if_sprite_exists(new_atlas, new_pos.x or 0, new_pos.y or 0)
	if has_sprite and new_pos.y and new_pos.y > 0 then
		...
		if
			(enhancement_1_z_order == nil and enhancement_2_z_order == nil)
			or (enhancement_1_z_order == nil and enhancement_2_z_order ~= nil and enhancement_2_z_order < 0)
			or (enhancement_2_z_order == nil and enhancement_1_z_order ~= nil and enhancement_1_z_order < 0)
		then
			...
			local enhancement_1_colour = aij_get_mcc_pixel(
				enhancement_1_atlas.image_data,
				center.pos,
				{ bpx = enhancement_1_atlas.px, bpy = enhancement_1_atlas.py, check_invis = false }
			)
			local enhancement_2_colour = aij_get_mcc_pixel(
				enhancement_2_atlas.image_data,
				other_center.pos,
				{ bpx = enhancement_2_atlas.px, bpy = enhancement_2_atlas.py, check_invis = false }
			)
			...
			if (v1 > 0.95 and s1 < 0.05) or (v2 > 0.95 and s2 < 0.05) then
				...
				local foreground_atlas =
					aij_recolour_atlas(
						{ 1, 1, 1, 1 },
						{ 1, 1, 1, 1 },
						foreground_atlas,
						nil,
						{ return_pixel = remove_white, skip_check = true }
					),
					foreground_enhancement.pos
				...
				if not SMODS.get_atlas(new_atlas_name) then
					...
					local image, image_data = process_texture_stack_enhancement_foreground(
						background_atlas.image,
						foreground_enhancement.key,
						foreground_atlas.name
					)
				...
				local s_base_low, s_base_high = aij_get_saturation_range(
					base_atlas.image_data,
					enhancement_to_recolour.pos,
					{ bpx = base_atlas.px, bpy = base_atlas.py }
				)
				local s_other_low, s_other_high = aij_get_saturation_range(
					other_atlas.image_data,
					other_enhancement.pos,
					{ bpx = other_atlas.px, bpy = other_atlas.py }
				)
				...
				local set_hue = function(r, g, b, a, old_colour, new_colour, args)
					...
					local s_merged = ((s_old - s_base_low) / (s_base_high - s_base_low)) * (s_other_high - s_other_low)
						+ s_other_low
				...
				return aij_recolour_atlas(
					old_colour,
					new_colour,
					base_atlas,
					nil,
					{ return_pixel = set_hue, skip_check = true }
				),
					enhancement_to_recolour.pos
			end
		else
			...
			if not SMODS.get_atlas(new_atlas_name) then
				...
				G[atlas_type][new_atlas_name] = {}
				SMODS.get_atlas(new_atlas_name).name = base_atlas.name
					.. "_aij_foreground_"
					.. foreground_enhancement.key
