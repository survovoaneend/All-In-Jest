...
All_in_Jest.PolyStakeChip = SMODS.GameObject:extend({
    ...
    check_for_unlock = function(self)
        ...
        G.PROFILES[G.SETTINGS.profile].aij_poly_chips_unlocked = G.PROFILES[G.SETTINGS.profile].aij_poly_chips_unlocked or {}
        if G.PROFILES[G.SETTINGS.profile].aij_poly_chips_unlocked[self.key] then return true end
        if not G.PROFILES[G.SETTINGS.profile].aij_poly_points then return false end
            ...
            {n=G.UIT.R, config={align = "cm", r = 0.1}, nodes={
                ...
                {n=G.UIT.R, config={align = "cm", padding = 0.04}, nodes={
                    {n=G.UIT.R, config={align = "cm", maxw = 3.4}, nodes={
                        {n=G.UIT.T, config={text = localize('k_stake'), scale = 0.5, colour = G.C.FILTER, shadow = true}},
                    }},
                    {n=G.UIT.R, config={align = "cm", maxw = 3.4}, nodes={
...
function All_in_Jest.create_polychip_unlock(chip)
    G.your_collection = CardArea(
        G.ROOM.T.x + G.ROOM.T.w/2,G.ROOM.T.h, 1.5, 1.5,
        {card_limit = 2, type = 'consumeable', highlight_limit = 0}
    )
  
    local chip_card = Card(G.your_collection.T.x + G.your_collection.T.w/2 - G.CARD_W/2, G.your_collection.T.y, 1.5, 1.5, G.P_CARDS.empty, G.P_CENTERS.j_joker, {aij_poly_chip = chip.key})
    ...
    chip_card.children.center = SMODS.create_sprite(0, 0, 1, 1, chip.atlas, chip.pos, chip_card.config.center.sprite_args)
    ...
    local locked_card = Card(G.your_collection.T.x + G.your_collection.T.w/2 - G.CARD_W/2, G.your_collection.T.y, 1.5, 1.5, G.P_CARDS.empty, G.j_locked)
    locked_card:remove_UI()
    locked_card.ID = chip_card.ID
    locked_card.children.center:remove()
    locked_card.children.center = SMODS.create_sprite(0, 0, 1, 1, 'locked_stake', {x=0, y=0}, locked_card.config.center.sprite_args)
    ...
    G.E_MANAGER:add_event(Event({timer = 'REAL',blockable = false,blocking = false,
        func = (function() G.OVERLAY_MENU.joker_unlock_table = chip_card.ID return true end) }))
    G.E_MANAGER:add_event(Event({timer = 'REAL',blockable = false,blocking = false, trigger = 'after', delay = 0.6,
        func = (function() if G.OVERLAY_MENU and G.OVERLAY_MENU.joker_unlock_table == chip_card.ID then locked_card:juice_up(0.3, 0.2); play_sound('cancel', 0.8) end; return true end) }))
    G.E_MANAGER:add_event(Event({timer = 'REAL',blockable = false,blocking = false, trigger = 'after', delay = 1.15,
        func = (function() if G.OVERLAY_MENU and G.OVERLAY_MENU.joker_unlock_table == chip_card.ID then locked_card:juice_up(0.45, 0.3); play_sound('cancel', 0.92) end; return true end) }))
    G.E_MANAGER:add_event(Event({timer = 'REAL',blockable = false,blocking = false, trigger = 'after', delay = 1.8,
        func = (function() if G.OVERLAY_MENU and G.OVERLAY_MENU.joker_unlock_table == chip_card.ID then locked_card:juice_up(0.6, 0.4); play_sound('cancel', 1.03) end; return true end) }))
    ...
    G.E_MANAGER:add_event(Event({
        ...
        delay = 2.3,
        func = (function() 
        if G.OVERLAY_MENU and G.OVERLAY_MENU.joker_unlock_table == chip_card.ID then locked_card:start_dissolve({G.C.BLACK}) end
        return true end)
    }))
    G.E_MANAGER:add_event(Event({
        timer = 'REAL',
    ...
    func = (function() if G.OVERLAY_MENU and G.OVERLAY_MENU.joker_unlock_table == chip_card.ID then chip_card.no_ui = nil; play_sound('timpani', 0.8, 1.8) end return true end) }))
    G.E_MANAGER:add_event(Event({timer = 'REAL',blockable = false,blocking = false, trigger = 'after', delay = 2.95,
    func = (function() if G.OVERLAY_MENU and G.OVERLAY_MENU.joker_unlock_table == chip_card.ID then play_sound('timpani', 1, 1.8)  end return true end) }))
    
    G.your_collection:emplace(chip_card)
    G.your_collection:emplace(locked_card)

    local desc_nodes = {}
    localize({type = 'descriptions', set = 'PolyStakeChips', key = 'locked', nodes = desc_nodes, vars = {chip.point_requirement}})
    ...
    local t = create_UIBox_generic_options({padding = 0,back_label = localize('b_continue'), no_pip = true, snap_back = true, back_func = 'continue_unlock', minw = 4.5, contents = {
        {n=G.UIT.R, config={align = "cm", padding = 0}, nodes={
            {n=G.UIT.R, config={align = "cm", padding = 0.1}, nodes={
                {n=G.UIT.R, config={align = "cm", padding = 0.1}, nodes={
                    {n=G.UIT.R, config={align = "cm", padding = 0}, nodes={
                        {n=G.UIT.O, config={object = DynaText({string = {localize('k_stake')}, colours = { G.C.BLUE }, shadow = true, rotate = true, bump = true, pop_in = 0.3, pop_in_rate = 2, scale = 1.2})}}
                    }},
                    {n=G.UIT.R, config={align = "cm", padding = 0}, nodes={
