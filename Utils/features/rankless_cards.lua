...
function get_front_spriteinfo(_front)
	if _front.card and _front.card.ability and _front.card.ability.numbertaker_rankless and _front.suit then
		if
			G.SETTINGS.colour_palettes[_front.suit] == "hc" and G.all_in_jest.numbertaker_rankless["hc_" .. _front.suit]
		then
			return G.ASSET_ATLAS[G.all_in_jest.numbertaker_rankless["hc_" .. _front.suit].atlas],
				G.all_in_jest.numbertaker_rankless["hc_" .. _front.suit].pos
		elseif G.all_in_jest.numbertaker_rankless[_front.suit] then
			return G.ASSET_ATLAS[G.all_in_jest.numbertaker_rankless[_front.suit].atlas],
				G.all_in_jest.numbertaker_rankless[_front.suit].pos
		else
...
function Card:set_sprites(_center, _front)
	...
	if self.ability and self.ability.numbertaker_rankless and _front and _front.suit then
		if not G.all_in_jest.numbertaker_rankless[_front.suit] then
			...
			local lpx, lpy =
				G.ASSET_ATLAS[G.all_in_jest.numbertaker_rankless["Other"].atlas].px,
				G.ASSET_ATLAS[G.all_in_jest.numbertaker_rankless["Other"].atlas].py
			...
			aij_pasteAlpha(
				...
				pos,
				{ x = 9, y = 4 },
				{ reverse = true, lpx = lpx, lpy = lpy, bpx = bpx, bpy = bpy }
			)
			...
			if new_color then
				new_color[4] = 255
				self.children.front.atlas =
					aij_recolour_atlas(replace_color, new_color, self.children.front.atlas, true)
			end
