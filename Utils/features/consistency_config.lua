...
function SMODS.poll_object(args)
	...
	if
		rate > 0
		and args.type == "Joker"
		and (args.append == "sho" or args.append == "buf")
		and not G.GAME.aij_found_mult
		and not args.attributes
		and not (#SMODS.find_card("j_aij_little_boy_blue") > 0)
	then
...
function get_new_boss()
	...
	if not All_in_Jest.config.suit_boss_blocking then
		return boss_key
	end
