SMODS.PokerHand({
	...
	evaluate = function(parts, hand)
		if not next(parts._flush) or not next(parts._straight) then
			return {}
		end
		local min = 10
		for j = 1, #hand do
			if hand[j]:get_id() < min then
				min = hand[j]:get_id()
			end
		end
		if min >= 10 then
...
function create_UIBox_hand_tip(handname)
	...
	if
		...
		and G.GAME.hands["aij_Royal Flush"]
		and G.GAME.hands["aij_Royal Flush"].level > G.GAME.hands["Straight Flush"].level
	then
		ret = {
			n = G.UIT.R,
			...
			nodes = {
				ret,
				{
					...
					config = {
						...
						force_focus = true,
						on_demand_tooltip = {
							text = localize("aij_Royal Flush", "poker_hand_descriptions"),
							filler = { func = create_UIBox_hand_tip, args = "aij_Royal Flush" },
						},
					},
					nodes = {
						{
							...
							nodes = {
								{
									...
									config = {
										...
										padding = 0.01,
										r = 0.1,
										colour = G.C.HAND_LEVELS[math.min(
											7,
											math.max(
												G.GAME.hands["aij_Royal Flush"].level
									...
									nodes = {
										{
											n = G.UIT.T,
											config = {
												text = " " .. localize("aij_Royal Flush", "poker_hands"),
												scale = 0.45,
									...
									nodes = {
										{
											...
											config = {
												text = "+" .. number_format(
													G.GAME.hands["aij_Royal Flush"].chips
								...
								{ n = G.UIT.T, config = { text = "X", scale = 0.45, colour = G.C.MULT } },
								{
									...
									nodes = {
										{ n = G.UIT.B, config = { w = 0.08, h = 0.01 } },
										{
											...
											config = {
												text = "+" .. number_format(
													G.GAME.hands["aij_Royal Flush"].mult
						...
						},
						{
							...
							nodes = {
								{
									n = G.UIT.T,
									config = { text = "  #", scale = 0.45, colour = G.C.UI.TEXT_LIGHT, shadow = true },
								},
							},
						},
						{
							...
							nodes = {
								{
									n = G.UIT.T,
									config = {
										text = G.GAME.hands["aij_Royal Flush"].played,
										scale = 0.45,
										colour = G.C.FILTER,
										shadow = true,
									},
