	...
	if center_to_copy.name == "To Do List" then
		if copied_base_stats.to_do_poker_hand then
			...
			while not ability_table.to_do_poker_hand do
				ability_table.to_do_poker_hand = pseudorandom_element(
					_poker_hands,
					pseudoseed(
						(copier_card.area and copier_card.area.config.type == "title") and "false_to_do" or "to_do"
					)
				)
				if ability_table.to_do_poker_hand == old_hand then
					ability_table.to_do_poker_hand = nil
				end
			end
		end
...
All_in_Jest.use_copied_joker_function = function(
	card,
	modded_func_name,
	vanilla_func_name,
	modded_func_args,
	...
	local to_copy = nil
	if
		card.ability[card.config.center.key].copied_joker_abilities
		and card.ability[card.config.center.key].copied_joker_abilities[index]
	then
	...
	if to_copy ~= nil then
		...
		if index ~= nil then
			if not skip_events and (#G.E_MANAGER.queues.base > starting_queue_length) then
				...
				event_insert_to_queue(
					G.E_MANAGER,
					starting_queue_length + 1,
					Event({
						func = function()
							expected_queue_length = #G.E_MANAGER.queues.base
								- (expected_queue_length - starting_queue_length)
								- 1
							All_in_Jest.hotswap_copied_ability(card, index)
							return true
						end,
...
All_in_Jest.set_copied_joker = function(copier_card, copied_center)
	...
	if had_prev_ability then
		...
		All_in_Jest.use_copied_joker_function(
			copier_card,
			"remove_from_deck",
			"remove_from_deck",
			{ copier_card, true },
		...
		if copied_center.key ~= nil and copied_center.key ~= copier_card.config.center.key then
			...
			if copied_center.dongtong_compat ~= false then
				jest_ability_calculate(
					{ ability = new_ability },
					"*",
					(copier_card.ability[copier_card.config.center.key].silver_multiplier_buff or 100)
						/ 100
						/ (2 ^ (new_ability.jest_applied and new_ability.jest_applied.j_aij_dongtong or 0)),
					{
						x_chips = 1,
						x_mult = 1,
						extra_value = true,
						rarity = true,
						[copier_card.config.center.key] = true,
					},
					nil,
					true,
					nil,
			...
			if
				not skip_funcs
				and #copier_card.ability[copier_card.config.center.key].copied_joker_abilities
					> copier_card.ability[copier_card.config.center.key].copy_limit
			then
				local index = #copier_card.ability[copier_card.config.center.key].copied_joker_abilities
					- copier_card.ability[copier_card.config.center.key].copy_limit
				local center_to_copy =
					G.P_CENTERS[copier_card.ability[copier_card.config.center.key].copied_joker_abilities[index].key]
			...
			if not (skip_funcs or copied_center[copier_card.config.center.key .. "_compat"] == false) then
				if copied_center[copier_card.config.center.key .. "_compat"] ~= false then
					All_in_Jest.hotswap_copied_ability(
						copier_card,
						#copier_card.ability[copier_card.config.center.key].copied_joker_abilities
					)
					copier_card.added_to_deck = false
					All_in_Jest.use_copied_joker_function(
						copier_card,
...
All_in_Jest.single_copier = SMODS.Joker:extend({
	...
	loc_vars = function(self, info_queue, card)
		if
			card.ability
			and card.config
			and card.ability[card.config.center.key]
			and card.ability[card.config.center.key].copied_joker_key ~= nil
		then
			local copied_center = G.P_CENTERS[card.ability[card.config.center.key].copied_joker_key]
			local info_queue_center =
				{ -- Create a simplified "fake" center that can be used without referencing/modifying the actual center object
	...
	calc_dollar_bonus = function(self, card)
		return table.unpack(
			All_in_Jest.use_copied_joker_function(card, "calc_dollar_bonus", "calculate_dollar_bonus", { card }, {})
		)
	end,
})
...
All_in_Jest.multi_copier = SMODS.Joker:extend({
	add_to_deck = function(self, card, from_debuff)
		if
			...
			and card.ability[card.config.center.key].copied_joker_abilities
			and #card.ability[card.config.center.key].copied_joker_abilities > 0
			and card.ability.name == card.config.center.key
		then
			for index = #card.ability[card.config.center.key].copied_joker_abilities, math.max(
				1,
				#card.ability[card.config.center.key].copied_joker_abilities
					- card.ability[card.config.center.key].copy_limit
					+ 1
			), -1 do
				local center_to_copy =
					G.P_CENTERS[card.ability[card.config.center.key].copied_joker_abilities[index].key]
		...
		then
			...
			for index = #card.ability[card.config.center.key].copied_joker_abilities, math.max(
				...
				if center_to_copy[card.config.center.key .. "_compat"] ~= false then
					...
					-- Make blueprint compatibility work
					if
						card.config.center.key ~= new_ability.key
						and not (context.blueprint and not G.P_CENTERS[new_ability.key].blueprint_compat)
					then
						local ret = table.unpack(
							All_in_Jest.use_copied_joker_function(
								card,
								"calculate",
								"calculate_joker",
		...
		if area.cards then
			for _, v in pairs(area.cards) do
				if v and type(v) == "table" and v.ability[v.config.center.key] and (count_debuffed or not v.debuff) then
					...
					if
						v.ability[v.config.center.key].copied_joker_key == key
						and G.P_CENTERS[key][key .. "_compat"] ~= false
					then
						table.insert(results, v)
					end
					-- Check if a multi-copier is copying it
					if
						type(v.ability[v.config.center.key].copied_joker_abilities) == "table"
						and v.ability[v.config.center.key].copy_limit
					then
						for i = #v.ability[v.config.center.key].copied_joker_abilities, math.max(
							1,
							#v.ability[v.config.center.key].copied_joker_abilities
								- v.ability[v.config.center.key].copy_limit
								+ 1
						), -1 do
							if
								v.ability[v.config.center.key].copied_joker_abilities[i]
								and v.ability[v.config.center.key].copied_joker_abilities[i].key == key
								and G.P_CENTERS[key][key .. "_compat"] ~= false
							then
								table.insert(results, v)
							end
