			...
			for k, v in pairs(G.P_CENTER_POOLS.Edition) do
				if edition.key == v.key and v.shader then
					if type(v.draw) == "function" then
						...
						G.shared_seals[self.seal]:draw_shader(
							v.shader,
							nil,
	...
	func = function(self, layer)
		...
		if self.ability.patches then
			for k, v in pairs(self.ability.patches) do
				if G.all_in_jest.patches_sprites["hc_" .. k] and G.SETTINGS.colour_palettes[k] == "hc" then
					...
					G.all_in_jest.patches_sprites["hc_" .. k]:draw_shader(
						"dissolve",
						nil,
						nil,
						nil,
		...
		if self.ability and self.ability.all_in_jest and self.ability.all_in_jest.perma_debuff then
			if G.all_in_jest.extra_card_sprites["Mark_of_the_Spear"].role then
				...
				G.all_in_jest.extra_card_sprites["Mark_of_the_Spear"]:draw_shader(
					"dissolve",
					nil,
					nil,
					nil,
		...
		if
			(
				(self.ability and self.ability.set and self.ability.set == "aij_astral")
				or (center and center.all_in_jest and center.all_in_jest.as_astral)
			) and not self.config.center.is_pin
		then
			if
				G.all_in_jest
				and G.all_in_jest.astral_grades[self.ability.consumeable.grade]
				and self.area
				and not self.area.config.collection
			then
				G.all_in_jest.astral_grades[self.ability.consumeable.grade].role.draw_major = self
				G.all_in_jest.astral_grades[self.ability.consumeable.grade]:draw_shader(
					"dissolve",
					nil,
					nil,
					nil,
	...
	func = function(self)
		if self.config.center and self.config.center.all_in_jest and self.config.center.all_in_jest.soul_layers then
			for k, v in pairs(self.config.center.all_in_jest.soul_layers) do
				local scale_mod = self.config.center.all_in_jest.soul_layers[k].moving
						and 0.07 + 0.02 * math.cos(1.8 * G.TIMERS.REAL) + 0.00 * math.cos(
							(G.TIMERS.REAL - math.floor(G.TIMERS.REAL)) * math.pi * 14
						) * (1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL))) ^ 3
					or 0.07
				local rotate_mod = self.config.center.all_in_jest.soul_layers[k].moving
				...
				if self.children[k] then
					...
					self.children[k]:draw_shader("dissolve", nil, nil, nil, self.children.center, scale_mod, rotate_mod)
				else
					local center = self.config.center
