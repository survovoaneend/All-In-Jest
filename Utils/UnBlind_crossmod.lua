...
function UnBlind_create_UIBox_blind(type) -- Main definition for the whole of the shop_sign replacement
	if type == "aij_Hidden" then
		local hidden_blind_animation =
			SMODS.create_sprite(0, 0, 0.75, 0.75, "aij_blind_drawn_replacement", { x = 0, y = 0 })
		hidden_blind_animation:define_draw_steps({
		...
		local t = --mix_colours(G.C.BLACK, G.C.L_BLACK, 0.5)		--G.C.DYN_UI.MAIN (red)		--G.C.DYN_UI.DARK (very similar to boss_main)		--black is too close to boss_main			--l_dark is too light
			{
				...
				config = {
					...
					r = 0.1,
					outline = 1,
					outline_colour = G.C.DYN_UI.BOSS_MAIN,
				},
				nodes = {
					{
						n = G.UIT.R,
						config = { align = "cm", padding = 0.09 },
						nodes = {
							{
								n = G.UIT.C,
								config = { id = "blind_extras", align = "cl" },
								nodes = {
									{
										n = G.UIT.R,
										config = { id = "tag_container", align = "cm" },
										nodes = {
											{
												n = G.UIT.R,
												config = {
													id = "empty_tag_replacement",
													align = "cm",
													r = 0.1,
													padding = 0.05,
													can_collide = true,
												...
												nodes = {
													{
														n = G.UIT.B,
														config = { id = "tag_desc", align = "cm", w = 0.75, h = 0.1 },
														nodes = {},
													},
												},
											},
								...
								nodes = {
									{
										...
										nodes = {
											...
											{
												n = G.UIT.O,
												config = { object = hidden_blind_animation, focus_with_object = true },
											},
										},
									},
								},
...
function UnBlind_create_UIBox_blind_tag(blind_choice) --Renders the tag that's availavle if the blind is skipped_rank	--called in main
	if G.GAME.all_in_jest.blind_tags.has_multiple and G.GAME.all_in_jest.blind_tags.amt > 1 then
		...
		if not G.GAME.all_in_jest.blind_tags[blind_choice] then
			return nil
		end
		...
		return {
			n = G.UIT.R,
			config = { id = "tag_container", ref_table = _tag, align = "cm" },
			nodes = {
				{
					n = G.UIT.R,
					config = {
