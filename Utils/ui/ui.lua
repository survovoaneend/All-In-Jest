...
function aij_get_UIE_by_fob(self, id, node)
	...
	if node.config and (node.config.func == id or node.config.button == id) then
		return node
	end
...
G.FUNCS.jest_tag_choice_next_page = function(e)
	...
	if blind_choice == G.GAME.blind_on_deck then
		G.GAME.all_in_jest.blind_tags.selected_index = nil
	end
	G.GAME.all_in_jest.blind_tags[blind_choice].page = G.GAME.all_in_jest.blind_tags[blind_choice].page
		+ e.config.ref_table[1]
...
SMODS.jest_no_back_card_collection_UIBox = function(_pool, rows, args)
	...
	for j = 1, #rows do
		if cards_per_page >= #pool and args.collapse_single_page then
			...
			G.your_collection[j] = CardArea(
				G.ROOM.T.x + 0.2 * G.ROOM.T.w / 2 + args.offset.x,
				G.ROOM.T.h + args.offset.y,
		...
		for j = 1, #rows do
			for i = 1, rows[j] do
				...
				if args.create_card then
					...
					card = args.from_area and copy_card(center, nil, args.card_scale)
						or Card(
				...
				if args.juice_card then
					card:juice_up(unpack(args.juice_card))
				end
	...
	local t = create_UIBox_generic_options({
		back_func = (args and args.back_func)
			or G.ACTIVE_MOD_UI and "openModUI_" .. G.ACTIVE_MOD_UI.id
			or "your_collection",
			...
			func = function()
				...
				if e.config.data[2].copies and e.config.data[2].copies > 1 then
					for i = 1, e.config.data[2].copies do
						...
						if
							(not card.edition or (card.edition and not card.edition.negative))
							and e.config.data[1] == G.consumeables
						then
			...
			func = function()
				...
				G.FUNCS.overlay_menu({
					...
					definition = SMODS.jest_no_back_card_collection_UIBox(
						...
						{
							e.config.data[2].size.x[e.config.data[2].times],
							e.config.data[2].size.y[e.config.data[2].times],
						},
						{
							no_materialize = true,
...
G.FUNCS.jest_next_tag = function(e)
	...
	if _tag then
		...
		if G.GAME.all_in_jest.skip_tags._tag_2 and G.GAME.all_in_jest.skip_tags._tag_3 then
			if
				_tag.config.ref_table.key ~= G.GAME.all_in_jest.skip_tags._tag_2._tag.key
				and _tag.config.ref_table.key ~= G.GAME.all_in_jest.skip_tags._tag_3._tag.key
			then
			...
			elseif _tag.config.ref_table.key == G.GAME.all_in_jest.skip_tags._tag_2._tag.key then
				...
				G.GAME.all_in_jest.skip_tags._tag_ui, G.GAME.all_in_jest.skip_tags._tag_sprite = temp_tag:generate_UI()
			elseif
				_tag.config.ref_table.key == G.GAME.all_in_jest.skip_tags._tag_3._tag.key
				and G.GAME.all_in_jest.skip_tags._tag_1
			then
...
function jest_create_select_card_ui(card, area, extra_data, select_func)
	...
	local t2 = {
		...
		config = {
			...
			padding = 0.1,
			align = "bm",
			colour = extra_data.alt_colour or G.C.GREEN,
			shadow = true,
			r = 0.08,
			minh = 0.3,
			one_press = true,
			button = select_func,
			data = { area, extra_data },
			hover = true,
		},
		nodes = {
			{
				n = G.UIT.T,
				config = { text = extra_data.alt_text or localize("k_aij_select"), colour = G.C.WHITE, scale = 0.5 },
			},
		},
	}
...
G.FUNCS.All_in_Jest_select_tag = function(e)
	...
	if G.GAME.all_in_jest.blind_tags.selected_index == number then
		...
		if G.GAME.round_resets.blind_tags[G.GAME.blind_on_deck] then
			G.GAME.round_resets.blind_tags[G.GAME.blind_on_deck] =
				G.GAME.all_in_jest.blind_tags[G.GAME.blind_on_deck][number]
		end
	end
end
...
G.FUNCS.aij_reroll_tag_button = function(e)
	...
	if
		((G.GAME.dollars - G.GAME.bankrupt_at) - G.GAME.current_round.reroll_cost >= 0)
