...
All_in_Jest.custom_ui = function(mod_nodes)
	...
	for i, center in pairs(set) do
		...
		local card = Card(
			G.aij_main_jokers_list.T.x + (G.aij_main_jokers_list.T.w / 2),
			G.aij_main_jokers_list.T.y,
			G.CARD_W,
			G.CARD_H,
			G.P_CARDS.empty,
			G.P_CENTERS[center] or All_in_Jest.DescriptionDummies[center],
			{
				...
				playing_card = i,
				viewing_back = false,
				bypass_back = G.P_CENTERS["b_aij_fabled"].pos,
			}
		)
