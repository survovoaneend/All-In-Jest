...
create_UIBox_your_collection_stickers = function()
	...
	return {
		...
		config = {
			...
			id = "your_collection_stickers_contents",
			align = "cm",
		},
	}
end
...
All_in_Jest.stickers_collection_content_UIBox = function(page, rows, args)
	...
	G.FUNCS.SMODS_card_collection_page = function(e)
		...
		for j = 1, #rows do
			for i = 1, rows[j] do
				...
				local center = not G.GAME.jest_pc_stickers_tab and (args.center and G.P_CENTERS[args.center])
				...
				if args.modify_card then
					args.modify_card(card, sticker, i, j)
				end
	...
	local t = create_UIBox_generic_options({
		...
		contents = {
			...
			(not All_in_Jest.config.aij_lite) and {
				n = G.UIT.R,
				config = {
					align = "cm",
					r = 0.1,
				...
				nodes = {
					{
						...
						nodes = {
							{
								n = G.UIT.T,
								config = { text = localize("aij_pc_stickers"), scale = 0.5, colour = G.C.UI.TEXT_LIGHT },
							},
						},
					},
				},
