...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    if (uv.y + misprint.y + texture_coords.y + image_details.y == uv.y)
        uv.y = misprint.y;
        uv.y = uv.y + texture_coords.y + image_details.y;
        
    //number low = min(pixel.r, min(pixel.g, pixel.b));
    //number high = max(pixel.r, max(pixel.g, pixel.b));
	//number delta = high-low -0.1;
    //
    //number fac = 0.8 + 0.9*sin(11.*uv.x+4.32*uv.y + misprint.r*12. + cos(misprint.r*5.3 + uv.y*4.2 - uv.x*4.));
    //number fac2 = 0.5 + 0.5*sin(8.*uv.x+2.32*uv.y + misprint.r*5. - cos(misprint.r*2.3 + uv.x*8.2));
    //number fac3 = 0.5 + 0.5*sin(10.*uv.x+5.32*uv.y + misprint.r*6.111 + sin(misprint.r*5.3 + uv.y*3.2));
    //number fac4 = 0.5 + 0.5*sin(3.*uv.x+2.32*uv.y + misprint.r*8.111 + sin(misprint.r*1.3 + uv.y*11.2));
    //number fac5 = sin(0.9*16.*uv.x+5.32*uv.y + misprint.r*12. + cos(misprint.r*5.3 + uv.y*4.2 - uv.x*4.));
    //
    //number maxfac = 0.7*max(max(fac, max(fac2, max(fac3,0.0))) + (fac+fac2+fac3*fac4), 0.);
    
    //colour.rgb = colour.rgb*0.5 + vec3(0.4, 0.4, 0.8);
    
    //colour.r = colour.r-delta + delta*maxfac*(0.7 + fac5*0.27) - 0.1;
    //colour.g = colour.g-delta + delta*maxfac*(0.7 - fac5*0.27) - 0.1;
    //colour.b = colour.b-delta + delta*maxfac*0.7 - 0.1;
    //colour.a = colour.a*(0.5*max(min(1., max(0.,0.3*max(low*0.2, delta)+ min(max(maxfac*0.1,0.), 0.4)) ), 0.) + 0.15*maxfac*(0.1+delta));
    //vec4 temp = pixel;
    
    
    return dissolve_mask(return_pixel, texture_coords, uv);
}
